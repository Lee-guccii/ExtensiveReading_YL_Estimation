...
###############
#textをnew_listに読み込む
with open("CohMetrixOutput(ippann).txt", "r", encoding="utf-8") as f:
    list = f.readlines()
...
#x = [5.0, 6.5, 7.0, 8.0, 8.5, 7.0, 8.0, 5.0, 5.0, 8.0, 5.5, 7.5, 8.0, 8.0, 7.0, 8.0, 8.0, 
#5.0, 8.0, 5.0, 8.0, 8.5, 5.5, 8.0, 5.5, 8.0, 9.5]

x = [5.0, 6.5, 7.0, 8.0, 8.5, 7.0, 8.0, 5.0, 5.0, 8.0, 5.5, 7.5, 8.0, 8.0, 7.0, 8.0, 8.0, 
5.0, 8.0, 5.0, 5.5, 8.0, 5.5, 9.5]
...
for key, value in leveldic.items():
    ...
    while count_level < 107:
        if (new_list[count_level][1].lower()).startswith(key.lower()):
            ...
            suuti_list = new_list[count_level][3:27] #試し追加分
            #文字列を数値に変換
